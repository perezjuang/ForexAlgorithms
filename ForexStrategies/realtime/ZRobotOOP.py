...
class ZRobotOOP(threading.Thread):
    ...
    def Update(self):
        ...
        self.pricedata_stadistics['per_k'] = (self.pricedata_stadistics['bidclose'] - self.pricedata_stadistics[
            'n_low']) * 100 / (
        ...
        if iRSI[len(iRSI) - 1] <= 40 and self.crossesOver(data_per_d,
                                                          self.pricedata_stadistics[
                                                              'lower_sto']) and lv_signal <= self.macdsub:
            self.logMessages = self.logMessages + "\n" + ("	 SEÑAL DE COMPRA ! \n")
            self.logMessages = self.logMessages + "\n" + ('''        
                  __,_,
                  [_|_/ 
        ...
        if iRSI[len(iRSI) - 1] >= 60 and self.crossesUnder(data_per_d,
                                                           self.pricedata_stadistics[
                                                               'upper_sto']) and lv_signal >= self.macduper:
            self.logMessages = self.logMessages + "\n" + "	  SEÑAL DE VENTA ! \n"
            self.logMessages = self.logMessages + "\n" + ('''
                   __
               _  |@@|
        ...
        # Cerrar Ventas #########################################
        if self.operacionventa and self.crossesOver(data_per_d, self.pricedata_stadistics['lower_sto']):
            if self.countOpenTrades("S") > 0:
                self.logMessages = self.logMessages + "\n" + "	  Cerrando Ventas Abiertas..."
                self.operacionventa = False
                self.exit("S")
        ...
        # Cerrar Compras #########################################
        if self.operacioncompra and self.crossesUnder(data_per_d, self.pricedata_stadistics['upper_sto']):
            if self.countOpenTrades("B") > 0:
