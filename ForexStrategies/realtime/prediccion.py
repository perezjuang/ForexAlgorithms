...
def recursive_multistep_model_fn(features, labels, mode, params=None):
    ...
    rnn_cell = tf.nn.rnn_cell.LSTMCell(num_hidden,
                                       state_is_tuple=False)  # https://www.tensorflow.org/api_docs/python/tf/contrib/rnn/LSTMCell
    ...
    if not predict_multi_steps_ahead:  # Classical singlestep ahead "mode":
        ...
        predictions = predictions[:, sequence_length - 1:,
                      :]  # assuming that the task is to *only* deliver the ahead-predictions
        predictions_dict = {"predictions": predictions}
...
singlestep_forecaster = tf.estimator.Estimator(model_fn=recursive_multistep_model_fn, model_dir=model_dir,
...
recursive_forecaster = tf.estimator.Estimator(model_fn=recursive_multistep_model_fn, model_dir=model_dir,
                                              params={'predict_steps_ahead': target_length_test,
...
# Create the Estimator:
joint_forecaster = tf.estimator.Estimator(model_fn=joint_multistep_model_fn,
                                          params={'num_hidden': num_hidden, 'targets_length': target_length_test})
...
for forecaster in [recursive_forecaster, joint_forecaster]:
    ...
    padding.fill(None)
    predictions_plot = np.concatenate(
        [np.concatenate([padding, np.squeeze(item["predictions"], axis=1)]) for item in predictions_raw])
