...
lineRegrbidClose, = ax1.plot([], [], label='Regresion Lineal Precio ' + timeframe)
...
def UpdatePlotter():
    ...
    # lineEmaFast.set_data(pricedata_stadistics['date_timestamp'].values, pricedata_stadistics['emaFast'].values)
    # lineEmaSlow.set_data(pricedata_stadistics['date_timestamp'].values, pricedata_stadistics['emaSlow'].values)
...
def Update():
    ...
    iFastSMA = sma(pricedata_stadistics['bidclose'], fast_sma_periods)
    iSlowSMA = sma(pricedata_stadistics['bidclose'], slow_sma_periods)
    ...
    angle = math.atan2(y, x) * (180.0 / math.pi)
    # angle = np.rad2deg(np.arctan2(vy[-1] - vy[0], vx[-1] - vx[0]))
